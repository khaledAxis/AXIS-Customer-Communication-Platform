...
- Next.js builds with `output: "standalone"`. The traced server, its traced runtime
  ...
  and started with Electron's isolated Node `utilityProcess`. Utility processes do not
  resolve modules from ASAR, so their `NODE_PATH` points only at that external traced
